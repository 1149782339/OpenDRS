...
import io.opendrs.sink.filter.ColumnFilter;
import io.opendrs.sink.jdbc.PreparedStatementBinder;
import io.opendrs.sink.jdbc.StatementBinder;
import io.opendrs.sink.naming.ColumnNamingStrategy;
import io.opendrs.sink.relation.ColumnDefinition;
...
import io.opendrs.sink.sql.ExpressionBuilder;
import io.opendrs.sink.sql.IdentifierRule;
import io.opendrs.sink.sql.SQLState;
import io.opendrs.sink.util.DateTimeUtil;
import io.opendrs.sink.util.StringUtil;
...
public abstract class CommonDatabaseDialect implements DatabaseDialect {
    ...
    /**
     * Connection supplier opened by the service (or a test).
     */
    ...
    public CommonDatabaseDialect(SinkConfig config) {
    ...
    public boolean tableExists(Connection connection, TableId tableId) throws SQLException {
        ...
        try (ResultSet resultSet = databaseMetaData.getTables(catalog, schema, table, new String[] {"TABLE"})) {
            ...
            while (resultSet.next()) {
                boolean catalogMatch = catalog == null || Objects.equals(catalog, resultSet.getObject(1));
                boolean schemaMatch = schema == null || Objects.equals(schema, resultSet.getObject(2));
                boolean tableMatch = Objects.equals(table, resultSet.getObject(3));
                if (catalogMatch && schemaMatch && tableMatch) {
                    return true;
                }
            }
            return false;
    ...
    public Connection getConnection() throws SQLException {
        if (connectionProvider == null) {
            throw new ConnectException("sink connection provider is not configured");
        }
